    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            if isinstance(m, v10Detect):
                def forward(x):
                    out = self.forward(x)["one2many"]
                    # 兼容原版 YOLOv10 和你现在新增 offset 分支后的结构
                    if isinstance(out, dict):
                        return out["feats"]
                    return out

            m.stride = torch.tensor([s / xi.shape[-2] for xi in forward(torch.zeros(1, ch, s, s))])  # forward
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    base_modules = frozenset({
        ...
        SCDown,
        C2fCIB,
        SubPixelUpsample,   # 新增：让它按 [c1, c2, ...] 规则自动改写参数
    })
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            # 分类头的输出等于 nc，不缩放；其余模块按 width 缩放
            if m is not Classify and c2 != nc:
